...
function McpUploadPage() {
    ...
    useEffect(() => {
        ...
        if (!user) {
            void showAlert('请先登录后再编辑 MCP 资源')
            navigate('/resource/mcp', { replace: true })
            return
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (sourceUrl.trim()) {
            try {
                ...
                if (!/^https?:$/i.test(parsed.protocol)) {
                    await showAlert('GitHub 链接仅支持 http/https')
                    return
                }
            } catch {
                await showAlert('GitHub 链接格式无效')
                return
            }
        }
        ...
        try {
            if (isEditMode) {
                const updated = await updateResourceFromUpload(editId, formData, 'mcp')
                navigate(`/resource/mcp/${updated.id}`, { replace: true })
            } else {
                const result = await uploadSkill(formData)
                navigate(`/resource/mcp/${result.skill.id}`, { replace: true })
            }
        } catch (err) {
    ...
    return (
        <div className="upload-page page-enter">
            <div className="upload-topbar upload-topbar-left">
                <Link to="/resource/mcp" className="upload-back-btn">← {t('upload.back')}</Link>
            </div>

            <div className="glass-card" style={{ padding: '32px' }}>
                <header style={{ marginBottom: '32px' }}>
                    <h2 style={{ fontSize: '2rem', marginBottom: '8px' }}>{isEditMode ? '编辑 MCP 能力' : '发布 MCP 能力'}</h2>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
                        {isEditMode ? '已为你预填历史内容，确认修改后提交即可。' : '分享您的 MCP 解决方案，支持 Markdown 富文本与图片。'}
                    </p>
                </header>
                ...
                <form className="upload-modern-layout" onSubmit={handleSubmit}>
                    <div className="modern-form-section">
                        ...
                        <div className="modern-field">
                            <label className="modern-label">GitHub 链接（可选）</label>
                            <input
                                className="modern-input"
                                value={sourceUrl}
                        ...
                        <div className="modern-field">
                            <label className="modern-label">{t('upload.uploadThumbnail')}</label>
                        ...
                        <div className="modern-field">
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                <label className="modern-label">正文内容 (Markdown)</label>
                                <button
                                    type="button"
                        ...
                        <button className="modern-submit-btn" disabled={uploading || prefillLoading || !name.trim() || !description.trim()}>
                            {uploading ? t('upload.reviewing') : (isEditMode ? '保存 MCP 修改' : '立即发布 MCP')}
                        </button>
                    </div>
